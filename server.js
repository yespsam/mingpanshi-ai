...
const CHAT_SYSTEM_PROMPT = [
  ...
  "每次回答都要像一位克制、专业、有温度的顾问：先准确复述用户真正关心的点，再给命理依据，再拆心理动力，最后给可执行步骤。",
  "固定使用 5 个小段落，段首分别为：【先听你说】【命盘依据】【心理动力】【行动建议】【自我提问】。",
  "【先听你说】不要空泛安慰，要把用户问题翻译成一个真实处境或内在冲突。",
  "【命盘依据】必须引用至少 2 类资料：四柱/日主、五行强弱、八卦六爻/变爻、星术参照、流年主题；不要只写“运势不错”。",
  "【心理动力】可使用咨询式提问、情绪命名、认知-情绪-行为链路、边界感、价值澄清；不能诊断疾病，不能自称治疗。",
  "【行动建议】给 2-3 个能在 7 天内执行的小步骤，尽量包含观察指标、沟通方式或风险边界。",
  "【自我提问】给 1-2 个尖锐但温和的问题，帮助用户继续思考。",
  "如果用户表达自伤、轻生或立即危险，先建议联系当地紧急服务、身边可信任的人或线下专业支持，再做简短陪伴。",
  "回答要具体、克制、可执行，不恐吓用户，不做确定性命运断言。",
  "不要提供医疗、法律、投资等专业结论；涉及钱、健康、重大关系时提醒用户自行判断。",
  "回答用中文纯文本，不要 Markdown 表格，不要编号堆砌，长度控制在 480-680 字。",
].join("\n");
...
function buildChatPrompt({ profile, report, history, message }) {
  ...
  return [
    ...
    [
      ...
      "请固定输出 5 段：",
      "【先听你说】识别用户真正焦虑、期待或纠结的点。",
      "【命盘依据】结合日主/四柱、五行强弱、六爻卦象/变爻、星术参照或流年中至少 2 类证据。",
      "【心理动力】解释可能的核心需要、压力模式、边界议题或认知-情绪-行为链路。",
      "【行动建议】给 2-3 个 7 天内能做的小动作，并说明观察什么反馈。",
      "【自我提问】给 1-2 个能帮助用户继续判断的问题。",
...
function localChatReply({ profile, report, message }) {
  ...
  return [
    `【先听你说】\n你问的是“${question || focus}”。这不只是想知道吉凶，更像是在确认：眼前这件事值不值得投入、该不该继续等、以及怎样做才不让自己后悔。报告主线是：${summary || `${strongest.element}较显、${weakest.element}需要补足。`}`,
    `【命盘依据】\n${solarNote}日主落在${profile?.pillars?.day?.name || "日柱"}，五行最强为${strongest.element}、最弱为${weakest.element}，说明你可调用的力量和容易卡住的地方都比较清楚。卦象为${primary.name || "本卦"}之${changed.name || "变卦"}，${sixYao.movement || "有动爻变化"}，重点在${sixYao.lineTheme || "变化位置"}；这表示局面不是固定结论，而是会随沟通、准备和节奏变化。${firstFlow.year ? `${firstFlow.year} 年流年主题是${firstFlow.theme}，适合分阶段验证。` : ""}`,
    `【心理动力】\n你更深层可能在意${psychology.coreNeed || "安全感、价值感或方向感"}，压力上来时容易${psychology.stressPattern || "把不确定感放大"}。这不是“想太多”，而是内在系统在提醒你：需要更多证据、边界和可控感。${stellar.sign ? `${stellar.sign}的星术参照也提示你可以用“${stellar.practice}”稳定节奏。` : ""}`,
    "【行动建议】\n先做三件小事：第一，把你能控制的部分写成 3 条行动，不写对方或环境必须怎样；第二，设一个 7 天内能验证的小动作，例如一次沟通、一次资料整理、一次试探性推进；第三，涉及钱、健康或长期关系时，至少用两条现实证据复核，不在情绪高点立刻拍板。",
